...
public class OfferController extends AbstractController {
	...
	@RequestMapping(value = "/user/offer/{offerId}", method = RequestMethod.GET, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<GenericResponse<JobOfferStatusDTO>> getOfferById(@PathVariable long offerId) {
		try {
			JobOfferStatusDTO offerDTO = this.userService.getOfferById(offerId);
			if(offerDTO != null) {
				return new ResponseEntity<GenericResponse<JobOfferStatusDTO>>(new GenericResponse<JobOfferStatusDTO>(offerDTO,HttpStatus.OK.value()), HttpStatus.OK);
			} else {
				return new ResponseEntity<GenericResponse<JobOfferStatusDTO>>(new GenericResponse<JobOfferStatusDTO>(null,HttpStatus.OK.value()), HttpStatus.OK);
			}
		} catch (Exception e) {
			LoggerManager.error(this, e);
